...
| **所属类** | F2FPayController |
| **模块** | roncoo-pay-web-gateway |
...
| **所属类** | RpTradePaymentManagerServiceImpl |
| **模块** | roncoo-pay-service |
...
| **数据写入** | 创建 RpTradePaymentOrder 对象（不直接写库） |

### sealRpTradePaymentRecord()

| 属性 | 内容 |
|------|------|
| **分类** | supporting |
| **职责** | 封装支付记录实体，生成trxNo/bankOrderNo，计算平台收入/成本/利润 |
| **数据写入** | 创建 RpTradePaymentRecord 对象（不直接写库） |
...
| 方法 | 所属模块 | 用途 | 被调用方 |
|------|---------|------|---------|
| WeiXinPayUtil.micropay() | trade/utils | 调用微信刷卡支付API | getF2FPayResultVo |
| AliPayUtil.tradePay() | trade/utils | 调用支付宝条码支付API | getF2FPayResultVo |
| MerchantApiUtil.getSign() | trade/utils | 商户API MD5签名 | getF2FPayResultVo |
