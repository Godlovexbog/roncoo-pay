...
生成精简路由型 CLAUDE.md 到**项目根目录**，只做导航不复制内容。

**硬性限制（违反即为失败）**：

| 规则 | 要求 |
|------|------|
| 总行数 | ≤200 行（不含 GitNexus 自动注入的 `<!-- gitnexus:* -->` 块） |
| 每章行数 | 除架构图和启动命令外，每章 ≤15 行 |
| 表格 | 只允许：模块速览表（≤10行）、启动命令表（≤10行）、配置文件速查表（≤8行）。其他章节**禁止表格** |
| "怎么跑" | 只保留编译命令+数据库初始化+启动命令表+冒烟 URL。不要复制环境要求说明、数据库连接详情 |
| "外部依赖" | **只列名称**，不超过 5 行。格式："微信支付、支付宝。MySQL、ActiveMQ、Ehcache。"。**禁止**列出连接字符串、队列名、配置路径 |

**章节模板**：

```
## 项目定位
(1-2 句) → 链接 .fingerprint.md

## 核心架构
(ASCII 分层图) → 链接 architecture.md

## 模块地图
(子域表 ≤8行 + 模块速览表 ≤10行) → 链接 architecture.md

## 关键约定
(命名模式 3行 + 分层调用链 1段 + 关键约束 5条) → 链接 config.md

## 怎么跑
(编译命令 + 数据库初始化 + 启动命令表 + 冒烟 URL) → 链接 env/setup.md

## 外部依赖
(支付通道名称 + 中间件名称, 共3-5行) → 链接 external-deps.md

## 禁区 / 历史包袱
(留空, 待人工补充)

## 相关文档
(panorama/ 下所有文件链接, 每行一个)
```

**自检清单**（agent 输出 CLAUDE.md 前必须逐条确认）：
- [ ] 总行数 ≤200？
- [ ] "外部依赖"章节有无表格？（有→删除表格）
- [ ] "怎么跑"章节有无环境要求说明块？（有→删除）
- [ ] 密码/连接字符串有无出现在 CLAUDE.md 中？（有→删除）
